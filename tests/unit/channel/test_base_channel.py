...
def test__repr(sync_cisco_iosxe_conn):
    assert (
        ...
        == "scrapli Channel {'logger': <Logger channel-localhost (WARNING)>, 'comms_prompt_pattern': '(^["
        "a-z0-9.\\\\-@()/:]{1,63}>$)|(^[a-z0-9.\\\\-@/:]{1,63}#$)|(^[a-z0-9.\\\\-@/:]{1,63}\\\\(conf["
        "a-z0-9.\\\\-@/:]{0,32}\\\\)#$)', 'comms_return_char': '\\n', 'comms_ansi': True, 'comms_auto_expand': "
        "False, 'timeout_ops': 30}"
    )
