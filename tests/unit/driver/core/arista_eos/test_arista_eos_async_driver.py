...
async def test_on_open(monkeypatch, async_eos_driver):
    ...
    async_eos_driver._current_priv_level = async_eos_driver.privilege_levels["privilege_exec"]
...
async def test_register_and_abort_config(monkeypatch, async_eos_driver):
    ...
    async_eos_driver.register_configuration_session(session_name="scrapli")
    async_eos_driver._current_priv_level = async_eos_driver.privilege_levels["scrapli"]
    await async_eos_driver._abort_config()

    assert async_eos_driver._current_priv_level.name == "privilege_exec"
