...
class TestDevice:
    ...
    def test_get_prompt(self, nix_conn, transport):
        ...
        assert prompt == self.test_cases["linux"]["get_prompt"]
    ...
    def test_send_command_short(self, nix_conn, transport, strip_prompt):
        command = self.test_cases["linux"]["send_command_short"]["command"]
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_response = self.test_cases["linux"]["send_command_short"][expected_type]
    ...
    def test_send_command_long(self, nix_conn, transport, strip_prompt):
        command = self.test_cases["linux"]["send_command_long"]["command"]
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_response = self.test_cases["linux"]["send_command_long"][expected_type]
        _, response = nix_conn.channel.send_input(channel_input=command, strip_prompt=strip_prompt)
        assert response.decode() == expected_response
...
class TestGenericDevice:
    ...
    def test_send_commands(self, nix_conn_generic, transport, strip_prompt):
        commands = []
        expected_responses = []
        commands.append(self.test_cases["linux"]["send_command_short"]["command"])
        commands.append(self.test_cases["linux"]["send_command_long"]["command"])
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_responses.append(self.test_cases["linux"]["send_command_short"][expected_type])
        expected_responses.append(self.test_cases["linux"]["send_command_long"][expected_type])
...
class TestNetworkDevice:
    ...
    def test_get_prompt_and_acquire_priv(self, conn, priv_level, device_type, transport):
        if self.test_cases[device_type]["get_prompt"][priv_level] is None:
        ...
        assert prompt == self.test_cases[device_type]["get_prompt"][priv_level]
    ...
    def test_send_command_short(self, conn, device_type, transport, strip_prompt):
        ...
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_response = self.test_cases[device_type]["send_command_short"][expected_type]
    ...
    def test_send_command_long(self, conn, device_type, transport, strip_prompt):
        ...
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_response = self.test_cases[device_type]["send_command_long"][expected_type]
        sanitize_response = self.test_cases[device_type]["sanitize_response"]
        response = conn.send_command(command=command, strip_prompt=strip_prompt)
        assert sanitize_response(response.result) == expected_response
    ...
    def test_send_commands(self, conn, device_type, transport, strip_prompt):
        commands = []
        expected_responses = []
        commands.append(self.test_cases[device_type]["send_command_short"]["command"])
        commands.append(self.test_cases[device_type]["send_command_long"]["command"])
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_responses.append(self.test_cases[device_type]["send_command_short"][expected_type])
        expected_responses.append(self.test_cases[device_type]["send_command_long"][expected_type])
        sanitize_response = self.test_cases[device_type]["sanitize_response"]
    ...
    def test_send_commands_from_file(self, conn, device_type, transport, strip_prompt):
        ...
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_responses = self.test_cases[device_type]["send_commands_from_file"][expected_type]
        sanitize_response = self.test_cases[device_type]["sanitize_response"]
    ...
    def test_send_interactive_normal_response(self, conn, device_type, transport):
        ...
        expected_response = self.test_cases[device_type]["send_interactive_normal_response"][
            "expected"
        ]
    ...
    def test_send_interactive_hidden_response(self, conn, device_type, transport):
        if self.test_cases[device_type]["send_interactive_hidden_response"] is None:
            pytest.skip(
                f"send_interactive_hidden_response for device type {device_type} not tested"
            )
        interact = self.test_cases[device_type]["send_interactive_hidden_response"]["command"]
        expected_response = self.test_cases[device_type]["send_interactive_hidden_response"][
            "expected"
        ]
        response = conn.send_interactive(interact_events=interact)
        assert response.result == expected_response
    ...
    def test_send_config(self, conn, device_type, transport, strip_prompt):
        ...
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_response = self.test_cases[device_type]["send_config"][expected_type]
        verification = self.test_cases[device_type]["send_config"]["verification"]
        expected_verification = self.test_cases[device_type]["send_config"][
    ...
    def test_send_configs(self, conn, device_type, transport, strip_prompt):
        ...
        expected_type = "expected_no_strip" if not strip_prompt else "expected_strip"
        expected_responses = self.test_cases[device_type]["send_configs"][expected_type]
        verification = self.test_cases[device_type]["send_configs"]["verification"]
        expected_verification = self.test_cases[device_type]["send_configs"][
            f"verification_{expected_type}"
        ]
        teardown_configs = self.test_cases[device_type]["send_configs"]["teardown_configs"]
        sanitize_response = self.test_cases[device_type]["sanitize_response"]
