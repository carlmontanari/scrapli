...
class AsyncChannel(Channel):
    async def _read_chunk(self) -> bytes:
        ...
        """
        fut = self.transport.stdout.read(65535)
        new_output = await asyncio.wait_for(fut=fut, timeout=2)
        # literally no matter what i do this shit returns a string??? wtf?
        new_output = new_output.encode()
        new_output = new_output.replace(b"\r", b"")
        if self.comms_ansi:
            new_output = strip_ansi(output=new_output)
        LOG.debug(f"Read: {repr(new_output)}")
    ...
    async def _read_until_input(self, channel_input: bytes, auto_expand: Optional[bool] = None) -> bytes:
        """
    ...
    async def send_input(self, channel_input: str, strip_prompt: bool = True,) -> Tuple[str, str]:
        ...
        raw_result, processed_result = await self._send_input(
            channel_input=channel_input, strip_prompt=strip_prompt
        )
        return raw_result.decode(), processed_result.decode()

    @operation_timeout("timeout_ops", "Timed out sending input to device.")
    async def _send_input(self, channel_input: str, strip_prompt: bool) -> Tuple[bytes, bytes]:
        """
